;(function () {
  ...
  USE(function (module) {
    ...
    var perf =
        typeof performance !== 'undefined'
          ...
          : false,
      start =
        (perf && perf.timing && perf.timing.navigationStart) || (perf = false)
    var S_ = (State._ = '>')
    State.drift = 0
  ...
  USE(function (module) {
    ...
    ;(function () {
      ...
      function chain(ctx, soul, key, val, state) {
        ...
        ;(put = ctx.put || (ctx.put = {}))[soul] = state_ify(
          put[soul],
          key,
          state,
          val,
    ...
    ;(function () {
      ...
      function verify(val, key, node, soul) {
        ...
        if (!state) {
          return (ctx.err =
            "Error: No state on '" + key + "' in node '" + soul + "'!")
        }
        ...
        ;(ctx.diff || (ctx.diff = {}))[soul] = Gun.state.to(
          node,
          key,
          ctx.diff[soul]
        )
  ...
  USE(function (module) {
    ...
    function relate(at, msg, from, rel) {
      ...
      tmp = from.id
        ? ((at.map || (at.map = {}))[from.id] = at.map[from.id] || { at: from })
        : {}
      if (rel === tmp.link) {
        if (!(tmp.pass || at.pass)) {
          return
  ...
  USE(function (module) {
    ...
    Gun.chain.get = function (key, cb, as) {
      ...
      if (typeof key === 'string') {
        if (key.length == 0) {
          ;(as = this.chain())._.err = {
            err: Gun.log('Invalid zero length string key!', key),
          }
          return null
        }
        var back = this,
  ...
  USE(function (module) {
    ...
    Gun.chain.put = function (data, cb, as) {
      ...
      if (as.soul || root === gun) {
        ...
        as.soul =
          as.soul ||
          (as.not =
            Gun.node.soul(as.data) ||
            (as.via.back('opt.uuid') || Gun.text.random)())
        as.via._.stun = {}
        if (!as.soul) {
      ...
      if (Gun.is(data)) {
        data.get(function (soul, o, msg) {
          if (!soul) {
            ...
            return Gun.log(
              'The reference you are saving is a',
              typeof msg.put,
              '"' + msg.put + '", not a node (object)!'
            )
          }
    ...
    function soul(id, as, msg, eve) {
      ...
      id = at.dub =
        at.dub ||
        id ||
        Gun.node.soul(cat.obj) ||
        Gun.node.soul(msg.put || at.put) ||
        Gun.val.link.is(msg.put || at.put) ||
        pat.join('/') /* || (function(){
      ...
      if (!as.not && !(as.soul = as.soul || soul)) {
        if (as.path && obj_is(as.data)) {
          ...
          as.soul =
            ...
            at.soul ||
            at.link ||
            (opt.uuid || as.via.back('opt.uuid') || Gun.text.random)()
        }
        if (!as.soul) {
          // polyfill async uuid for SEA
          as.via.back('opt.uuid')(function (err, soul) {
  ...
  USE(function (module) {
    ...
    function val(msg, eve, to) {
      ...
      if (
        ...
        u === data &&
        !at.root.opt.super &&
        eve.ack <= (opt.acks || Object.keys(at.root.opt.peers).length)
      ) {
        return
      }
      if (
        (!to &&
          (u === data || at.soul || at.link || (link && !(0 < link.ack)))) ||
        (u === data &&
          !at.root.opt.super &&
          (tmp = Object.keys(at.root.opt.peers).length) &&
          !to &&
          (link || at).ack < tmp)
      ) {
  ...
  USE(function (module) {
    ...
    function each(v, k) {
      ...
      ;((tmp = gun.get(k)._).echo || (tmp.echo = {}))[cat.id] =
        tmp.echo[cat.id] || cat
    }
    ...
    function Mesh(root) {
      ...
      var hear = (mesh.hear = function (raw, peer) {
        ...
        if ('{' === tmp || ((raw['#'] || obj_is(raw)) && (msg = raw))) {
          ...
          console.STAT &&
            (ST = +new Date() - S) > 9 &&
            console.STAT(S, ST, 'msg') // TODO: PERF: caught one > 1.5s on tgif
      ...
      ;(function () {
        ...
        var say = (mesh.say = function (msg, peer) {
          ...
          setTimeout(function () {
            console.STAT &&
              (ST = +new Date() - S) > 9 &&
              console.STAT(S, ST, '0ms TO', id, peer.id)
            flush(peer)
          }, opt.gap)
          send(raw, peer)
        })
