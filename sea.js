;(function () {
    ...
    try {
      if (SEA.window) {
        if (
          ...
          console.warn(
            'WebCrypto used by GUN SEA implementation does not work without HTTPS. Will automatically redirect.'
          )
  ...
  USE(function (module) {
    ...
    if (!api.crypto) {
      try {
        ...
        console.log(
          'text-encoding and @peculiar/webcrypto may not be included by default, please add it to your package.json!'
        )
      }
    }
  ...
  USE(function (module) {
    ...
    SEA.certify =
      ...
      */
          ...
          const readBlacklist =
            ((opt || {}).blacklist || {}).read &&
            (typeof opt.blacklist.read === 'string' || opt.blacklist.read['#'])
              ? opt.blacklist.read
              : null
          const writeBlacklist =
            typeof (opt || {}).blacklist === 'string' ||
            (((opt || {}).blacklist || {}).write || {})['#']
              ? opt.blacklist
              : ((opt || {}).blacklist || {}).write &&
                (typeof opt.blacklist.write === 'string' ||
                  opt.blacklist.write['#'])
              ? opt.blacklist.write
              : null
  ...
  USE(function (module) {
    ...
    var Gun =
      (SEA.window || {}).Gun ||
      USE((typeof MODULE == 'undefined' ? '.' : '') + './gun', 1)
    Gun.SEA = SEA
    SEA.GUN = SEA.Gun = Gun
  ...
  USE(function (module) {
    ...
    User.prototype.create = function (...args) {
      ...
      act.g = function (auth) {
        ...
        root
          .get('~@' + alias)
          .put(Gun.obj.put({}, tmp, Gun.val.link.ify(tmp)), act.h) // next up, we want to associate the alias with the public key. So we add it to the alias list.
      }
      act.h = function () {
        cat.ing = false
  ...
  USE(function (module) {
    ...
    check.pub = function (eve, msg, val, key, soul, at, no, user, pub) {
                    ...
                    return root
                      ...
                      .once((value) => {
                        if (value && (value === 1 || value === true))
                          return no('Certificant blacklisted.')
                        return cb(data)
                      })
                  }
                  return cb(data)
      ...
      if (
        ...
        !raw['+'] &&
        (pub === tmp.pub ||
          (pub !== tmp.pub && ((msg._.out || {}).opt || {}).cert))
      ) {
        SEA.sign(
          SEA.opt.pack(msg.put),
          user._.sea,
          function (data) {
